...
//
// configureDashboard.java is part of the JobsDashboard plugin.
//
    ...
    extends ComponentBase
{
    ...
    @Override public Widget doInit()
    {
        ...
        configureSaveHandler("/myUser/userSettings/myJobsDashboard");
        configureCancelHandler();

        renderCurrentProjectAndProcedureConfiguration("/myUser/userSettings/myJobsDashboard", true);
        return mainLaunch;
    }
            ...
            public void onClick(ClickEvent event) {
                ...
                dpReq.setCallback(new CommanderObjectCallback() {
                    ...
                    public void handleResponse(CommanderObject response) {
                        ...
                        SetPropertyRequest setProcedureColumnHeaderProperty = getRequestFactory().createSetPropertyRequest();
                        setProcedureColumnHeaderProperty.setPropertyName(propertySheetRoot+"/procedureColumnHeader");
                        setProcedureColumnHeaderProperty.setValue(col2Header.getText());
                        setProcedureColumnHeaderProperty.setCallback(new PropertyCallback() {
                            @Override
                            public void handleResponse(Property response) {
                                //To change body of implemented methods use File | Settings | File Templates.
                            }
                        ...
                        */
                        ...
                        //have any procedures been selected?
                        if(spArray.isEmpty()){
                            Window.alert("No procedures selected");
                            btnSave.setEnabled(true);
                            return;
                        }

                        //now that selected procedures have been added add the other properties
                        spArray.add(setDashboardTitleProperty);
                        spArray.add(setProcedureColumnHeaderProperty);
    ...
    private void processDashboardConfig(PropertySheet ps){
        for (Map.Entry<String, Property> entry : ps.getProperties().entrySet())
        {
            ...
            if (prop.isStringProperty())
            {
                ...
                if(prop.getName().toLowerCase()=="procedurecolumnheader"){
    ...
    private HorizontalPanel createProcedureRow(final String procedureName, Boolean selected){
     ...
     */
    private HorizontalPanel procedureExistsInTree(String procedureName, TreeItem procedureRoot){

        HorizontalPanel procedurePanel=null;

        int proceduresInList=procedureRoot.getChildCount();
        for(int i=0;i<procedureRoot.getChildCount();i++){
            HorizontalPanel hp = null;
            hp=(HorizontalPanel)procedureRoot.getChild(i).getWidget();
            if(hp!=null){
                //Window.alert(String.valueOf(hp.getWidgetCount()));
                if(hp.getWidgetCount()>1){
                    String currentProcedureName = ((Label)hp.getWidget(1)).getText();
                    if(currentProcedureName.toLowerCase()==procedureName.toLowerCase()){
                        procedurePanel=hp;
                        break;
                    }
                }
    ...
    private void populateProcedureList(final String projectName, final TreeItem parentItem, final Property propertySheet){
        GetProceduresRequest reqProcs = getRequestFactory().createGetProceduresRequest();
        reqProcs.setProjectName(projectName);

        reqProcs.setCallback(new ProcedureListCallback() {
            @Override
            public void handleResponse(List<Procedure> response) {
                //To change body of implemented methods use File | Settings | File Templates.
                ...
                for(Procedure proc:response){
                    HorizontalPanel hp = new HorizontalPanel();
                    hp=createProcedureRow(proc.getName(),false);
                ...
                if(propertySheet!=null){
                    ...
                    for (Map.Entry<String, Property> projectSheetEntry : propertySheet.getPropertySheet().getProperties().entrySet())
                    {
                        ...
                        HorizontalPanel hp = procedureExistsInTree(propertyValue,parentItem);
                        if(hp!=null){
                            CheckBox cb = (CheckBox)hp.getWidget(0);
                            cb.setValue(true);
                        }
    ...
    public void renderCurrentProjectAndProcedureConfiguration(String propertySheetPath, boolean recursive){
            ...
            public void onOpen(OpenEvent<TreeItem> treeItemOpenEvent) {
                ...
                if(ti.getChildCount()>0){
                    ...
                    if(ti.getChild(0).getWidget().getClass().getName().toLowerCase().contains("horizontalpanel")){
                        ...
                        if(l.getText().equalsIgnoreCase("loading procedures...")){
                            ti.getChild(0).remove();
                            String project=ti.getText();
                            populateProcedureList(project,ti,null);
            ...
            public void handleResponse(PropertySheet response) {
                ...
                for (Map.Entry<String, Property> entry : response.getProperties().entrySet())
                {
                    ...
                    if (prop.isStringProperty()){
                        ...
                        populateProcedureList(currentProjectName,currentProjectItem,prop);
                    }
                }
                ...
                for(Project currentProject:response){
                    if(currentProject.getPluginName().isEmpty()){
                        ...
                        if(projectLabel==null){
                            ...
                            HorizontalPanel loadingProceduresHp = createProcedureRow("Loading procedures...",false);
                            //ti.addItem(new Label("Loading procedures..."));
                            //hide checkbox
                            //loadingProceduresHp.getWidget(0).setVisible(false);
                            ti.addItem(loadingProceduresHp);
